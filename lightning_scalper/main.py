#!/usr/bin/env python3
"""
🚀 Lightning Scalper - Main Application Entry Point
...
class LightningScalperApp:
    """
    🚀 Lightning Scalper Main Application
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.logger.info(f"🚀 {self.app_name} v{self.version} initialized")
    ...
    def load_clients_configuration(self, config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if clients_file.exists():
                ...
                self.logger.info(f"✅ Loaded {len(clients_data)} client configurations")
                return clients_data.get('clients', [])
            else:
                self.logger.warning(f"⚠️ Clients config file {clients_file} not found")
            ...
            self.logger.error(f"❌ Failed to load clients configuration: {e}")
            return []
    ...
    def _create_sample_clients_file(self, clients_file: Path, config: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.info(f"📝 Created sample clients file: {clients_file}")
            
        except Exception as e:
            self.logger.error(f"❌ Failed to create sample clients file: {e}")
    ...
    async def startup(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # 2. Start core system
            self.logger.info("⚡ Starting core trading system...")
            success = await self.controller.start_system()
            
            if not success:
                self.logger.error("❌ Failed to start core trading system")
                return False
            ...
            if clients_data:
                self.logger.info(f"👥 Loading {len(clients_data)} clients...")
                ...
                for client_data in clients_data:
                    try:
                        ...
                        if config['application']['debug_mode']:
                            ...
                            self.logger.info(f"✅ Client {client_data['client_id']} registered (Debug Mode)")
                        else:
                            # Production mode - full MT5 integration
                            success = self.controller.add_client(client_data)
                            if success:
                                self.logger.info(f"✅ Client {client_data['client_id']} connected successfully")
                            else:
                                self.logger.warning(f"⚠️ Failed to connect client {client_data['client_id']}")
                    
                    except Exception as e:
                        self.logger.error(f"❌ Error loading client {client_data.get('client_id', 'Unknown')}: {e}")
            ...
            self.logger.info(f"   👥 Active clients: {system_status['metrics']['active_clients']}")
    ...
    async def _attempt_restart(self):
        ...
        try:
            ...
            self.logger.info(f"🔄 Attempting system restart (attempt {self.stats['restart_count']}/3)...")
    ...
    async def shutdown(self):
        ...
        try:
            ...
            self.logger.info(f"   ⏱️ Total runtime: {self.stats['total_runtime']}")
            self.logger.info(f"   🔄 Restart count: {self.stats['restart_count']}")
